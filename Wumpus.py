...
def applylogic_from_knowledge():
    ...
    for step in tl:
        print
        "_______________thinking for %s_________________" % (step)
        if step in visited:
            print
            step, "is visited so avoid thinking about that\n"
            ...
            print
            "Connections of", step, "checking for :", tl2
            # print present
            # print prev
            for st in tl2:
                if st == present:
                    ...
                    if st in start:
                        ...
                        if (k[int(st[0])][int(st[1])] in ['04', '40', '02', '20']) and (
                            ...
                            print
                            "Putting Safe State at ", int(step[0]), int(step[1])
                            print
                            "Visited :", visited
                            return step
                            print
                        ...
                        for objs in dg:
                            if objs in start:
                                print
                                "Diagonal", objs, "is Start state"
                            elif objs == present:
                                print
                                "Diagonal", objs, "is Present state"
                            elif objs in visited:
                                tv = 1
                                print
                                "%s is diagonal of %s which is visited" % (objs, st)
                            ...
                            if tv is not 1:
                                print
                                "here Nothing could be done for", st
    print
    "________________\n"
    return 0
